...
bool DateComparisonNode::Evaluate(const Date& d, const string& ev) { // вычислить звено с датами
...
bool EventComparisonNode::Evaluate(const Date& d, const string& ev) { // вычислить звено с событиями
	if (ev.empty()) {
		return 0;
	}
...
bool LogicalOperationNode::Evaluate(const Date& d, const string& ev) { // вычислить логическую операцию
